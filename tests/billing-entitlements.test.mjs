...
test('paid plan store and order entitlements are defined server-side and free validation access stays bounded',async()=>{
  ...
  assert.match(billing,/FREE_STORE_LIMIT=1/);
  assert.match(billing,/FREE_ORDER_LIMIT=500/);
  assert.match(billing,/ENTITLED_STATUSES[^\n]*trialing[^\n]*active[^\n]*past_due/);
  assert.match(billing,/return \{planKey:'free' as const,stores:FREE_STORE_LIMIT,orders:FREE_ORDER_LIMIT,entitled:false\}/);
  assert.match(billing,/return \{planKey:plan\.key,stores:plan\.stores,orders:plan\.orders,entitled:true\}/);
});

test('marketplace connection creation enforces the store entitlement atomically',async()=>{
...
test('billing summary exposes effective store and monthly order capacity without trusting the browser',async()=>{
  ...
  assert.match(summary,/billing_order_usage_daily/);
  assert.match(summary,/orders:ordersUsed/);
  assert.match(summary,/orderLimit:entitlement\.orders/);
  assert.match(summary,/canProcessOrders:ordersUsed<entitlement\.orders/);
  assert.match(summary,/orderUsageBasis/);

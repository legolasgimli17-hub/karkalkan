...
test('paid plan store entitlements are defined server-side and free validation access stays bounded',async()=>{
  ...
  assert.match(billing,/if\(!plan\|\|!ENTITLED_STATUSES\.has\(normalizedStatus\)\)return \{planKey:'free' as const,stores:FREE_STORE_LIMIT,entitled:false\}/);
});
...
test('billing summary exposes effective store capacity without trusting the browser',async()=>{
  ...
  assert.match(summary,/entitlement:\{planKey:entitlement\.planKey,stores:entitlement\.stores,entitled:entitlement\.entitled\}/);
});

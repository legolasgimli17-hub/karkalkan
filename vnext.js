...
function kkDateLabel(v){const d=new Date(v);return Number.isNaN(d.getTime())?'—':d.toLocaleDateString('tr-TR',{day:'2-digit',month:'short'})}
...
function kkMount(){
  ...
  if(nav){
    ...
    const setActive=()=>{let best=null,bestTop=Infinity;for(const a of all){const target=document.querySelector(a.getAttribute('href'));if(!target)continue;const top=Math.abs(target.getBoundingClientRect().top-120);if(top<bestTop){best=a;bestTop=top}}all.forEach(a=>a.classList.toggle('is-active',a===best))};
    window.addEventListener('scroll',setActive,{passive:true});setActive();
  }

  const command=document.createElement('div');command.className='kk-command-bar';command.innerHTML=`
    <article class="kk-command-main">
      <div class="kk-kicker">KârKalkan yaklaşımı</div>
      <h3 class="kk-command-title">Hızlı sipariş sinyali, sonradan doğrulanan finans gerçeği.</h3>
      <p class="kk-command-copy">Sipariş geldiğinde erken sinyal al; hakediş, kargo ve maliyet verisi geldikçe rakamın hangi bölümünün gerçekten doğrulandığını ayrı gör. Tek bir “net kâr” rakamına körü körüne güvenmek yerine kanıtı da yanında taşı.</p>
      <div class="kk-command-tags"><span>Satış kanıtı</span><span>İade kanıtı</span><span>Maliyet kapsamı</span><span>Kargo eşleşmesi</span><span>Hakediş düzeltmeleri</span></div>
    </article>
    <article class="kk-live-card">
      <div class="kk-live-head"><div><strong>Canlı sipariş sinyali</strong><small>Webhook hızlı haberdar eder; periyodik senkron finansı doğrular.</small></div><span id="kkWebhookState" class="kk-live-state"><i></i> Kontrol ediliyor</span></div>
      <div class="kk-live-actions"><button id="kkWebhookBtn" class="btn ghost compact-btn" type="button">Canlı akışı kur</button><span id="kkWebhookMessage" class="kk-live-note">Önce mağaza ve API bilgileri gerekir.</span></div>
    </article>`;
  syncMessage.after(command);

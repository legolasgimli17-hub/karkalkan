...
const SYNONYMS:Record<Target,string[]>={
  ...
  product_name:['product name','urun adi','ürün adı','urun ismi','ürün ismi','title','product title','item name','aciklama','açıklama'],
  ...
  gross_sales:['gross sales','sales','brut satis','brüt satış','satis tutari','satış tutarı','gross revenue','revenue','item price','sales amount'],
...
function deterministic(columns:Column[]){
  ...
  for(const column of columns){
    ...
    if(best&&bestScore>=.62){used.add(best);suggestions.push({source:column.name,target:best,confidence:Math.round(bestScore*100)/100,reason:bestScore>=.9?'Kolon adı güçlü eşleşiyor.':'Kolon adı ve veri tipi eşleşiyor.'})}
...
function validateModel(value:any,columns:Column[],fallback:Suggestion[]){
  ...
  for(const raw of value.mappings){
    ...
    if(!sourceNames.has(source)||!bySource.has(source))return null
...
async function improveWithModel(columns:Column[],fallback:Suggestion[]){
  ...
  const instructions='You map ecommerce finance CSV column schemas. You receive column names and aggregate type ratios only, never row values. Map only when meaning is supported. Do not map customer name/email/phone/address, bank account, tax identity or unrelated fields to a finance target. Use null when uncertain. Never invent a source column. One source and one target may be used at most once.'
  try{
...
Deno.serve(async(req:Request)=>{
  ...
  const fallback=deterministic(columns),result=await improveWithModel(columns,fallback)

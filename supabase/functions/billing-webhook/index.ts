...
function parseSignature(value:string){let ts='';const signatures:string[]=[];for(const part of value.split(';')){const [key,entry]=part.split('=',2);if(key==='ts')ts=entry||'';if(key==='h1'&&entry)signatures.push(entry)}return {ts,signatures}}
...
Deno.serve(async(req:Request)=>{
  ...
  const {data:existing}=await admin.from('billing_events').select('safe_error_code').eq('paddle_event_id',eventId).maybeSingle()
  if(existing&&!existing.safe_error_code)return plainJson(200,{received:true,duplicate:true})
  const eventRow={paddle_event_id:eventId,event_type:eventType,payload_sha256:await sha256Hex(raw),occurred_at:event?.occurred_at||null,safe_error_code:'PROCESSING'}
  ...
  try{
    ...
    if(eventType.startsWith('subscription.')&&userId){
      ...
      const customPlan=String(data?.custom_data?.plan_key||'')
      const planKey=isPlanKey(customPlan)?customPlan:isPlanKey(reversePlan)?reversePlan:'free'

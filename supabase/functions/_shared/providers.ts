...
export const PROVIDERS:Record<MarketplaceKey,{
  ...
  flo:{label:'FLO',mode:'file',tier:'gated',sellerIdLabel:'Tedarikçi / mağaza ID',sellerIdRequired:true,credentialFields:[{key:'api_username',label:'API kullanıcı adı',max:220},{key:'api_password',label:'API şifresi',max:320}],capabilities:['Standart finans raporu','Partner API kimlik kasası','Mağazayı yeniden kurmadan API geçişi'],note:'FLO partner API erişimini mağaza bazında verir. Tedarikçi ID, API kullanıcı adı ve API şifresi hazır değilse standart finans raporuyla çalışın.'}
}

...
Deno.serve(async(req:Request)=>{
  ...
  const hooks=await sql`select user_id,secret_hash,status from public.marketplace_webhooks where connection_id=${connectionId}::uuid limit 1`;const hook=hooks[0];if(!hook||hook.status!=='active'||String(hook.secret_hash)!==suppliedHash)return json(401,{error:'UNAUTHORIZED'})
